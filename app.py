...
def analyze():
    ...
    cursor.execute("SELECT age, heart_rate, symptoms FROM user_inputs")
    data = cursor.fetchall()

    # Convert data to DataFrame
    df = pd.DataFrame(data, columns=['age', 'heart_rate', 'symptoms'])

    # Preprocess the symptoms (example: simple encoding, should be improved)
    df['symptoms'] = df['symptoms'].astype('category').cat.codes

    # Features and labels (using age and heart_rate as features, symptoms as labels for example)
    X = df[['age', 'heart_rate']]
    y = df['symptoms']
    ...
    # Create a pie chart for symptom distribution
    plt.figure(figsize=(8, 8))
    symptom_counts = df['symptoms'].value_counts()
    plt.pie(symptom_counts.values, labels=symptom_counts.index, autopct='%1.1f%%', startangle=90)
    ...
    buf.seek(0)
    pie_plot_url = base64.b64encode(buf.getvalue()).decode('utf8')

    return render_template('analysis.html', accuracy=accuracy, pie_plot_url=pie_plot_url)
